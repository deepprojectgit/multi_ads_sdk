...
### Changed
- **iOS & Android**: Reduced native ad height across layouts (padding, icon size, media height, margins) for better component management.
- **Flutter example**: Reduced native ad slot heights (small: 96, medium: 280).

### Fixed
- **iOS**: AppDelegate Swift initialization—use `MobileAds.shared.start(completionHandler:)` instead of deprecated `GADMobileAds.sharedInstance().start(completionHandler: nil)`.
